...
def gan_ke(gan1, gan2):
    return (
        True
        if ten_deities[gan1]['克'] == ten_deities[gan2]['本'] or ten_deities[gan2]['克'] == ten_deities[gan1]['本']
        else False
    )
...
if not options.b:
    ...
    print(f'{sex}命', end=' ')
    print('\t公历:', end=' ')
    print(f'{solar.getYear()}年{solar.getMonth()}月{solar.getDay()}日', end=' ')
    yun = ba.getYun(not options.n)
    print('  农历:', end=' ')
    print(
        f'{lunar.getYear()}年{lunar.getMonth()}月{lunar.getDay()}日 穿=害 上运时间：{yun.getStartSolar().toFullString().split()[0]} 命宫:{ba.getMingGong()} 胎元:{ba.getTaiYuan()} 身宫:{ba.getShenGong()}\n',
        end=' ',
    )
    print(
        '\t',
        ...
        lunar.getPrevJieQi(True).getSolar().toYmdHms(),
...
print(
    '{1:{0}^15s}{2:{0}^15s}{3:{0}^15s}{4:{0}^15s}'.format(
        chr(12288),
        '【年】{}:{}{}{}'.format(
            temps[gans.year], temps[zhis.year], ten_deities[gans.year].inverse['建'], gan_zhi_he(zhus[0])
        ),
        '【月】{}:{}{}{}'.format(
            temps[gans.month], temps[zhis.month], ten_deities[gans.month].inverse['建'], gan_zhi_he(zhus[1])
        ),
        f'【日】{temps[me]}:{temps[zhis.day]}{gan_zhi_he(zhus[2])}',
        '【时】{}:{}{}{}'.format(
            temps[gans.time], temps[zhis.time], ten_deities[gans.time].inverse['建'], gan_zhi_he(zhus[3])
        ),
    )
)
print('-' * 120)
...
print(
    '\033[1;36;40m{1:{0}<15s}{2:{0}<15s}{3:{0}<15s}{4:{0}<15s}\033[0m'.format(
        chr(12288),
        f'{gans.year}{yinyang(gans.year)}{gan5[gans.year]}【{ten_deities[me][gans.year]}】{check_gan(gans.year, gans)}',
        f'{gans.month}{yinyang(gans.month)}{gan5[gans.month]}【{ten_deities[me][gans.month]}】{check_gan(gans.month, gans)}',
        f'{me}{yinyang(me)}{gan5[me]}{check_gan(me, gans)}',
        f'{gans.time}{yinyang(gans.time)}{gan5[gans.time]}【{ten_deities[me][gans.time]}】{check_gan(gans.time, gans)}',
    )
)
...
print(
    '\033[1;36;40m{1:{0}<15s}{2:{0}<15s}{3:{0}<15s}{4:{0}<15s}\033[0m'.format(
        chr(12288),
        f'{zhis.year}{yinyang(zhis.year)}{ten_deities[gans.year][zhis.year]}{ten_deities[gans.month][zhis.year]}【{ten_deities[me][zhis.year]}】{ten_deities[gans.time][zhis.year]}{get_empty(zhus[2], zhis.year)}',
        f'{zhis.month}{yinyang(zhis.month)}{ten_deities[gans.year][zhis.month]}{ten_deities[gans.month][zhis.month]}【{ten_deities[me][zhis.month]}】{ten_deities[gans.time][zhis.month]}{get_empty(zhus[2], zhis.month)}',
        f'{zhis.day}{yinyang(zhis.day)}{ten_deities[gans.year][zhis.day]}{ten_deities[gans.month][zhis.day]}【{ten_deities[me][zhis.day]}】{ten_deities[gans.time][zhis.day]}',
        f'{zhis.time}{yinyang(zhis.time)}{ten_deities[gans.year][zhis.time]}{ten_deities[gans.month][zhis.time]}【{ten_deities[me][zhis.time]}】{ten_deities[gans.time][zhis.time]}{get_empty(zhus[2], zhis.time)}',
    )
)
...
for seq, item in enumerate(zhis):
    ...
    for gan in zhi5[item]:
        out = out + f'{gan}{gan5[gan]}{ten_deities[me][gan]}　'
...
for seq, item in enumerate(zhus):
    ...
    # 检查元辰
    result = (
        '{}－{}'.format(result, '元辰') if zhis[seq] == Zhi[(Zhi.index(zhis[0]) + direction * -1 * 5) % 12] else result
    )
...
if not options.b:
    for dayun in yun.getDaYun()[1:]:
        ...
        for gan in zhi5[zhi_]:
            zhi5_ = zhi5_ + f'{gan}{ten_deities[me][gan]}　'
...
print('调候：', tiaohous[f'{me}{zhis[1]}'], '\t##金不换大运：', jinbuhuan[f'{me}{zhis[1]}'])
print('金不换大运：说明：', jins[f'{me}'])
...
liuqins = bidict({
    '才': '父亲',
    '财': '财' if options.n else '妻',
    '印': '母亲',
    '枭': '偏印' if options.n else '祖父',
...
for item in Gan:
    print(
        f'{item}:{ten_deities[me][item]} {liuqins[ten_deities[me][item]]}-{ten_deities[item][zhis[0]]} {ten_deities[item][zhis[1]]} {ten_deities[item][zhis[2]]} {ten_deities[item][zhis[3]]}',
        end='  ',
    )
    if Gan.index(item) == 4:
        print()
...
for item in gan_scores:
    print(f'{item}[{ten_deities[me][item]}]-{gan_scores[item]} ', end='  ')
print()
print('-' * 120)
yinyangs(zhis)
...
if '比' in gan_shens:
    ...
    if shens2.count('比') > 2 and '比' in zhi_shens:
        ...
        if ('官' not in shens) and ('杀' not in shens):
...
if me_lu == zhis[2]:
    ...
    if (zhi_xing[3] and gan_he[3] and gan_shens[3] == '财') or (
        zhi_xing[2] and gan_he[2] and zhi_xing[1] and gan_he[1] and gan_shens[1] == '财'
    ):
...
if (gan_shens[0] in ('财', '才') and gan_shens[1] in ('财', '才')) or (
    gan_shens[1] in ('财', '才') and ('财' in zhi_shen3[1] or '才' in zhi_shen3[1])
):
...
if ('财' not in shens2) and ('才' not in shens2):
...
if (gan_he[3] and gan_shens[3] == '财' and jin_jiao(zhis[2], zhis[3])) or (
    gan_he[2] and gan_he[1] and gan_shens[1] == '财' and jin_jiao(zhis[1], zhis[2])
):
...
if '官' in gan_shens:
    ...
    if ('财' not in gan_shens) and ('印' not in gan_shens):
...
if '食' in gan_shens:
    ...
    if ('财' not in gan_shens) and ('才' not in gan_shens):
...
if '伤' in gan_shens:
    ...
    if gan_shens[0] == '伤' and gan_shens[1] == '伤' and ('伤' not in zhi_shens2):
        print('年月天干都浮现伤官，亲属少。')
    ...
    for dayun in yun.getDaYun()[1:]:
        ...
        for liunian in dayun.getLiuNian():
            ...
            for gan in zhi5[zhi2_]:
                zhi6_ = zhi6_ + f'{gan}{ten_deities[me][gan]}　'
...
if nayins[zhus[0]][-1] == '火':
    if zhis.day in '戌亥':
        print(f'| 天罗：{zhis.day}', end=' ')

# 地网
if nayins[zhus[0]][-1] in '水土':
    if zhis.day in '辰巳':
        print(f'| 地网：{zhis.day}', end=' ')
...
if ge == '财' or ge == '才':
    ...
    if cai_num > 2 and ('劫' not in shens) and ('比' not in shens) and ('比' not in shens) and ('印' not in shens):
